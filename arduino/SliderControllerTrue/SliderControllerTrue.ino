...
// Pour la communication avec l'IHM
...
void loop()
{
  // Réception d'une commande de l'ihm
  if (Serial.available() > 0)
  {
    // read the oldest byte in the serial buffer:
    incomingString = Serial.readString();

    // Initialisation (première lettre est un i)
    ...
    // Start (première lettre est un v suivi de la consigne de vitesse)
...
void asservissement()
{
  ...
  analogWrite(PWM, cmd);
  if (_DEBUG)
    Serial.println(vitesse_lineaire_reelle, 8);
}
